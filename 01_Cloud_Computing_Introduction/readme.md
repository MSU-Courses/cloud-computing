...
- [Введение в облачные вычисления](#введение-в-облачные-вычисления)
  ...
  - [История эволюции вычислительных технологий](#история-эволюции-вычислительных-технологий)
    - [Что такое вычисления?](#что-такое-вычисления)
    - [От локальных вычислений к кластерам и Grid Computing](#от-локальных-вычислений-к-кластерам-и-grid-computing)
      - [Локальные вычисления](#локальные-вычисления)
      - [Кластерные вычисления](#кластерные-вычисления)
      - [Grid Computing](#grid-computing)
      - [Utility Computing](#utility-computing)
      - [Переход к облачным вычислениям](#переход-к-облачным-вычислениям)
  - [Определение облака и облачных вычислений](#определение-облака-и-облачных-вычислений)
  - [Ключевые характеристики облачных вычислений](#ключевые-характеристики-облачных-вычислений)
    - [Самообслуживание по запросу (On-demand self-service)](#самообслуживание-по-запросу-on-demand-self-service)
    - [Широкий сетевой доступ (Broad network access)](#широкий-сетевой-доступ-broad-network-access)
    - [Объединение ресурсов и мултиарендность (Resource pooling and multi-tenancy)](#объединение-ресурсов-и-мултиарендность-resource-pooling-and-multi-tenancy)
    - [Эластичность и масштабируемость (Elasticity and Scalability)](#эластичность-и-масштабируемость-elasticity-and-scalability)
    - [Измеряемый сервис и оплата по факту (Measured service)](#измеряемый-сервис-и-оплата-по-факту-measured-service)
    - [Отказоустойчивость (Fault Tolerance)](#отказоустойчивость-fault-tolerance)
    - [Вывод](#вывод)
  - [Локальная инфраструктура vs облако: основные различия](#локальная-инфраструктура-vs-облако-основные-различия)
    - [Контроль и ответственность](#контроль-и-ответственность)
    - [Стоимость и модель затрат](#стоимость-и-модель-затрат)
    - [Масштабируемость и гибкость](#масштабируемость-и-гибкость)
    - [Доступность и надежность](#доступность-и-надежность)
    - [Безопасность и соответствие требованиям](#безопасность-и-соответствие-требованиям)
    - [Скорость развертывания и обновления](#скорость-развертывания-и-обновления)
    - [Вывод](#вывод-1)
  - [Модели сервисов в облаке: IaaS, PaaS, SaaS](#модели-сервисов-в-облаке-iaas-paas-saas)
    - [Infrastructure as a Service (IaaS)](#infrastructure-as-a-service-iaas)
    - [PaaS (Platform as a Service)](#paas-platform-as-a-service)
    - [SaaS (Software as a Service)](#saas-software-as-a-service)
  - [Модели развертывания облаков: public, private, hybrid, community, multi-cloud](#модели-развертывания-облаков-public-private-hybrid-community-multi-cloud)
  - [Service Level Agreement (SLA) и Quality of Service (QoS)](#service-level-agreement-sla-и-quality-of-service-qos)
    - [SLA](#sla)
    - [QoS](#qos)
  - [Роли и профессии в сфере облачных технологий](#роли-и-профессии-в-сфере-облачных-технологий)
...
## Определение облака и облачных вычислений

Сам термин *облачные вычисления* (_cloud computing_) имеет несколько определений с разных точек зрения.

Согласно определению Национального института стандартов и технологий США (NIST): "*Облачные вычисления — это модель, обеспечивающая повсеместный, удобный и доступный по требованию сетевой доступ к общей совокупности настраиваемых вычислительных ресурсов (например, сетей, серверов, систем хранения, приложений и сервисов), которые могут быть быстро предоставлены и освобождены с минимальными управленческими усилиями или взаимодействием с провайдером.*".

Если говорить проще, *облачные вычисления* — это возможность пользоваться чужими серверами и программами через Интернет как готовой услугой. Вам не нужно покупать дорогое оборудование и заниматься его обслуживанием. Вместо этого вы берёте ресурсы "в аренду" у облачного провайдера

Важно различать термины "*облако*" и "*облачные вычисления*". Под *облаком* понимается сама инфраструктура провайдера: дата-центры с серверами, сетями и системами хранения данных, которые объединены и скрыты от пользователя. С точки зрения конечного пользователя, облако похоже на единый виртуальный компьютер, который можно использовать через интернет.

> Облако - это инфраструктура провайдера, а облачные вычисления - это модель использования этой инфраструктуры.
