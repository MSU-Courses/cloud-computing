# Облачные вычислительные сервисы и идентификация машин. Amazon AWS Elastic Compute Cloud
...
- [Облачные вычислительные сервисы и идентификация машин. Amazon AWS Elastic Compute Cloud](#облачные-вычислительные-сервисы-и-идентификация-машин-amazon-aws-elastic-compute-cloud)
...
1. Из каких строительных блоков состоит типичное облачное приложение и почему изучение облака удобно начинать с вычислений?
2. Какой облачный сервис нужен, если вы хотите, чтобы ваше приложение работало круглосуточно и было доступно пользователям?
3. Что означает каждая часть в имени типа экземпляра `m7g.large`?
4. Что произойдёт с данными на instance store, если экземпляр остановить и снова запустить?
5. Платите ли вы что-нибудь за остановленный экземпляр?
6. Почему класть access keys на сервер плохая идея, даже если репозиторий приватный?
7. Как приложение на EC2 получает права, если в его коде и настройках нет ни одного ключа?
8. Что такое Instance Metadata Service и чем IMDSv2 безопаснее IMDSv1?
9. В каких случаях выгоднее Spot-экземпляры, а в каких от них лучше отказаться?
...
![4.5. Вход на сервер по SSH-ключу](../_images/04/ssh_keys.png)

_4.5. Вход на сервер по SSH-ключу_
...
![4.6. Жизненный цикл экземпляра EC2](../_images/04/ec2_lifecycle.png)

_4.6. Жизненный цикл экземпляра EC2_
...
На данный момент данная архитектура выглядит следующим образом, по ходу данного курса мы будем её улучшать и дополнять новыми компонентами.

![4.7. Архитектура учебного сценария](../_images/04/first_architecture.png)

_4.7. Архитектура учебного сценария_
