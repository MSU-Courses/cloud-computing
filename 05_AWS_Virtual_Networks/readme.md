...
- [Виртуальные сети в облаке. Amazon VPC](#виртуальные-сети-в-облаке-amazon-vpc)
  ...
  - [Основы сетей](#основы-сетей)
    ...
    - [Подсети и IP-адреса (IPv4, IPv6)](#подсети-и-ip-адреса-ipv4-ipv6)
    ...
    - [Что такое шлюз (Gateway)](#что-такое-шлюз-gateway)
    - [Модель OSI](#модель-osi)
  - [Введение в виртуальные сети в AWS](#введение-в-виртуальные-сети-в-aws)
    - [Amazon VPC: Логически изолированная облачная сеть](#amazon-vpc-логически-изолированная-облачная-сеть)
    - [Подсети. Публичные и приватные подсети](#подсети-публичные-и-приватные-подсети)
    - [Диапазон VPC и ограничения CIDR](#диапазон-vpc-и-ограничения-cidr)
    - [Таблицы маршрутов и локальный маршрут](#таблицы-маршрутов-и-локальный-маршрут)
    - [Интернет-шлюз (Internet Gateway)](#интернет-шлюз-internet-gateway)
    - [NAT-шлюз (NAT Gateway) и доступ из приватных подсетей](#nat-шлюз-nat-gateway-и-доступ-из-приватных-подсетей)
    - [Public IP vs Private IP vs Elastic IP](#public-ip-vs-private-ip-vs-elastic-ip)
    - [VPC Peering](#vpc-peering)
    - [Общий VPC (VPC Sharing)](#общий-vpc-vpc-sharing)
    - [Site-to-Site VPN. Подключение VPC к локальной сети](#site-to-site-vpn-подключение-vpc-к-локальной-сети)
  - [Безопасность в VPC](#безопасность-в-vpc)
    - [Группы безопасности (Security Groups)](#группы-безопасности-security-groups)
    - [Сетевые ACL (Network ACL)](#сетевые-acl-network-acl)
    - [Группы безопасности vs Сетевые ACL](#группы-безопасности-vs-сетевые-acl)
  - [Дополнительные сервисы AWS для сетей](#дополнительные-сервисы-aws-для-сетей)
    - [AWS Route 53](#aws-route53)
    - [Понятие CDN. Amazon CloudFront](#понятие-cdn-amazon-cloudfront)
  - [Резюме](#резюме)
