...
Чтобы устройства находили друг друга, у каждого в сети есть свой _IP-адрес_ – уникальный числовой идентификатор. Напомним, сеть – это совокупность устройств, которые могут обмениваться данными, где IP-адрес это идентификатор устройства в этой сети.
...
_IP-адрес_ включает две части: _адрес сети_ и _адрес хоста_. _Адрес сети_ определяет, к какой сети принадлежит устройство, а _адрес хоста_ идентифицирует конкретное устройство в этой сети. Все устройства в одной сети имеют общий префикс адреса сети, но разные адреса хоста.

_Подсеть_ – это логически выделенная часть сети, в которой устройства объединены общим сетевым префиксом. По сути, подсеть – это «_сеть внутри сети_». Например, все устройства, подключенные к одному домашнему маршрутизатору, находятся в одной подсети и имеют одинаковый начальный фрагмент IP-адреса
...
Из каждой подсети *AWS резервирует 5 адресов для своих нужд*:
...
В реальных сетях пакет данных доходит до адресата по таблицам маршрутизации. Это таблицы, в которых указано, куда отправлять пакеты в зависимости от их назначения. Например, если пакет адресован узлу `10.0.1.5`, таблица укажет маршрут через интерфейс, связанный с подсетью `10.0.1.0/24`. Если же адрес назначения `10.0.2.5`, будет использован маршрут через интерфейс подсети `10.0.2.0/24`. Кроме того, почти всегда в таблице маршрутов присутствует шлюз по умолчанию (*default gateway*) — адрес, на который направляется весь трафик, если для него нет более специфичного маршрута.
...
Интернет-шлюз (IGW) – это компонент VPC, который обеспечивает связь между вашей VPC и глобальным интернетом. Он масштабируемый и отказоустойчивый, управляемый AWS ресурс. Когда вы подключаете IGW к своей VPC, вы как бы «открываете ворота» в интернет для этой сети.

Интернет-шлюз выполняет две основные функции:

1. Служит точкой выхода/входа в маршрутах (цель для маршрута 0.0.0.0/0),
2. Занимается Network Address Translation (NAT) – преобразует приватные IP ваших инстансов в публичные при общении с интернетом.

Например, если EC2-экземпляр имеет публичный IP, интернет-шлюз сопоставляет его с приватным IP экземпляра и пропускает трафик наружу и вовнутрь.

Чтобы сделать подсеть публичной, нужно: создать интернет-шлюз, прикрепить его к VPC и добавить маршрут в таблицу маршрутов подсети, направляющий внешний трафик на этот IGW. Также инстансу в этой подсети должен быть назначен публичный IP. После этого инстанс станет доступен из интернета (конечно, при правильных настройках безопасности).
...
Для частных подсетей AWS предлагает сервис _NAT Gateway_.

_NAT-шлюз в AWS_ – управляемый сервис, который предоставляет такой узел: он размещается в публичной подсети, имеет собственный публичный IP, и пропускает трафик из приватных подсетей в интернет, подменяя исходные адреса на свой. При этом обратные входящие соединения извне напрямую к приватным экземплярам невозможны – интернет-узлы «видят» только NAT-шлюз. Таким образом, приватные инстансы могут обновлять пакеты, обращаться к внешним API, но остаются недоступны из интернета напрямую.
...
![Рисунок 5. NAT Gateway для доступа из приватной подсети в интернет](../_images/05/07_nat_gateway.png)
...
![Рисунок 6. VPC Sharing: общий доступ к подсетям между аккаунтами AWS](../_images/05/08_vpc_sharing.png)

_Рисунок 6. VPC Sharing: общий доступ к подсетям между аккаунтами AWS_
...
![Рисунок 7. Site-to-Site VPN: подключение VPC к локальной сети](../_images/05/11_vps_security_groups.png)

_Рисунок 7. Site-to-Site VPN: подключение VPC к локальной сети_
...
![Рисунок 8. Пример Security Group в Amazon VPC](../_images/05/11_vps_security_groups.png)

_Рисунок 8. Пример Security Group в Amazon VPC_
...
![Рисунок 9. Пример Network ACL (NACL)](../_images/05/12_vpc_acl.png)

_Рисунок 9. Пример Network ACL (NACL)_
...
![Рисунок 3. Процесс разрешения DNS-запроса с использованием Amazon Route 53](../_images/05/04_dns_resolution.png)

_Рисунок 3. Процесс разрешения DNS-запроса с использованием Amazon Route 53_
...
![Рисунок 2. Географическое распределение edge-локаций и региональных кэшей Amazon CloudFront по всему миру](../_images/05/03_cloudfront_edges.png)

_Рисунок 2. Географическое распределение edge-локаций и региональных кэшей Amazon CloudFront по всему миру_
