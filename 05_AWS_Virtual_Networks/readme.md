...
1. _Точка входа и выхода для маршрутов_. IGW используется как целевой шлюз в таблице маршрутов (например, маршрут `0.0.0.0/0 → igw-id`).
2. _Подмена адресов_. IGW заменяет внутренние (приватные) адреса серверов на их внешние (публичные), чтобы они могли общаться с интернетом.

Например, у EC2 есть внутренний (приватный) адрес и внешний (публичный). Когда сервер общается с интернетом, IGW подставляет вместо приватного адреса публичный, а когда приходит ответ — возвращает его обратно на приватный адрес сервера.
...
Ниже представлена таблица сравнения *Internet Gateway* и *NAT Gateway*.
