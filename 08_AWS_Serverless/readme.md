...
<i>
В лаборатории снова горел свет. За окном лил дождь, и капли барабанили по стеклу в такт мигающим индикаторам серверов. Джон и Эмма смотрели на экран — их приложение работало, но с каждым днём нагрузка росла.

– Опять завис, – сказала Эмма, глядя на монитор. – Наш сервер не выдерживает пиков. Когда никто не заходит – простаивает, а когда нагрузка растёт – всё падает.

Джон устало провёл рукой по лицу. – Мы не можем больше масштабироваться вручную. Каждый день — как пожар.

Он открыл консоль AWS и набрал в поиске “Lambda”.

– А если попробовать без серверов вообще? – тихо сказал он. – Пусть код живёт сам по себе.

Эмма подняла бровь. – Как это “без серверов”? У нас же всё держится на них.

– Серверы всё ещё есть, – улыбнулся Джон, – просто мы перестаём ими управлять. Наш код запускается только тогда, когда приходит событие: пользователь зашёл, загрузил файл, отправил форму. И мы платим только за то, что реально работает.

В этот момент их монитор мигнул, и на экране появилась надпись: “Function deployed successfully.”

Джон нажал тестовый запуск. Функция выполнилась мгновенно. Серверная нагрузка — ноль.

– Работает… – прошептала Эмма. – И никаких серверов.

Но радость длилась недолго. Через минуту вспыхнуло уведомление: “Timeout error. Function exceeded 15 seconds.”

– Похоже, даже без серверов у нас есть границы, – усмехнулась она.

Джон рассмеялся. – Да, но теперь мы можем запускать тысячу таких функций одновременно. Пусть каждая делает своё дело. Это как оркестр, где каждый инструмент включается только тогда, когда наступает его партия.

Гул серверов за стеной больше не пугал их — он стал далёким эхом прошлого. Теперь их система жила сама по себе, просыпаясь только тогда, когда этого требовал мир.
</i>

## Содержание

- [Безсерверные вычисления. AWS Lambda](#безсерверные-вычисления-aws-lambda)
  - [Содержание](#содержание)
  - [Вопросы для самопроверки](#вопросы-для-самопроверки)
  - [Введение в Serverless Computing](#введение-в-serverless-computing)
    - [Традиционный подход к разработке приложений](#традиционный-подход-к-разработке-приложений)
    - [Концепция Serverless Computing](#концепция-serverless-computing)
    - [Основные характеристики концепции Serverless](#основные-характеристики-концепции-serverless)
    - [Serverless vs Server: ключевые различия](#serverless-vs-server-ключевые-различия)
    - [Преимущества и недостатки Serverless Computing](#преимущества-и-недостатки-serverless-computing)
  - [AWS Lambda. Введение и основные концепции](#aws-lambda-введение-и-основные-концепции)
    - [Что такое AWS Lambda?](#что-такое-aws-lambda)
    - [Назначение AWS Lambda](#назначение-aws-lambda)
    - [Масштабируемость AWS Lambda](#масштабируемость-aws-lambda)
    - [Событийно-ориентированная модель AWS Lambda](#событийно-ориентированная-модель-aws-lambda)
    - [AWS Lambda, как функция](#aws-lambda-как-функция)
    - [Поддерживаемые языки программирования](#поддерживаемые-языки-программирования)
    - [Модель оплаты AWS Lambda](#модель-оплаты-aws-lambda)
  - [Источники событий для AWS Lambda](#источники-событий-для-aws-lambda)
    - [Концепция триггеров и источников событий](#концепция-триггеров-и-источников-событий)
    - [Amazon S3 как источник событий](#amazon-s3-как-источник-событий)
    - [DynamoDB Streams как источник событий](#dynamodb-streams-как-источник-событий)
    - [API Gateway](#api-gateway)
    - [SQS, SNS](#sqs-sns)
    - [Другие сервисы](#другие-сервисы)
    - [Вызов через SDK, CLI и Console](#вызов-через-sdk-cli-и-console)
  - [Конфигурация функций AWS Lambda](#конфигурация-функций-aws-lambda)
    - [Компоненты функции](#компоненты-функции)
    - [Код функции и зависимости](#код-функции-и-зависимости)
    - [Название функции в коде](#название-функции-в-коде)
    - [Слои AWS Lambda (AWS Lambda Layers)](#слои-aws-lambda-aws-lambda-layers)
    - [Создание Lambda-функции через AWS Console](#создание-lambda-функции-через-aws-console)
      - [Шаг 1. Имя функции](#шаг-1-имя-функции)
      - [Шаг 2. Выбор runtime](#шаг-2-выбор-runtime)
      - [Шаг 3. Шаг 3. Execution Role (роль выполнения)](#шаг-3-шаг-3-execution-role-роль-выполнения)
      - [Шаг 4. Триггер](#шаг-4-триггер)
      - [Шаг 5. Function Code](#шаг-5-function-code)
      - [Шаг 6. Memory](#шаг-6-memory)
      - [Шаг 7. Environment Variables](#шаг-7-environment-variables)
      - [Шаг 8. Загрузка функции и зависимостей](#шаг-8-загрузка-функции-и-зависимостей)
  - [Ограничения AWS Lambda](#ограничения-aws-lambda)
    - [Soft Limits](#soft-limits)
    - [Hard Limits](#hard-limits)
  - [AWS Lambda vs Amazon EC2](#aws-lambda-vs-amazon-ec2)
    - [Различия и сценарии использования](#различия-и-сценарии-использования)
...
- _AWS Lambda_ — это облачный сервис от Amazon, позволяющий запускать код в ответ на события без необходимости управления серверами. Такой подход называется Serverless Computing.
- Serverless-архитектура обладает следующими преимуществами:
  - Автоматическое масштабирование. Запущенные функции автоматически обрабатывают любое количество запросов.
  - Платежи “pay-as-you-go”. Пользователь платит только за фактическое время выполнения функций.
  - Отсутствие необходимости поддержки инфраструктуры. AWS управляет серверами, обновлениями, масштабированием и резервированием.
- Основные сценарии использования AWS Lambda:
  - Обработка данных из Amazon S3 (например, реакции на загрузку файлов).
  - Реагирование на события из DynamoDB Streams.
  - Построение бэкендов для REST API (в связке с API Gateway).
  - Интеграция с другими сервисами (SQS, SNS и др.).
- Ключевые особенности AWS Lambda:
  - Поддерживает различные языки программирования (Python, Node.js, Java, Go и др.).
  - Возможность настройки окружения (память, таймаут, переменные среды).
  - Интеграция с Amazon CloudWatch для мониторинга, логирования и алёртов.
- Ограничения Lambda:
  - Максимальное время выполнения функции: 15 минут.
  - Оперативное потребление: до 10,240 МБ.
  - Размер пакета развёртывания ограничен (50 МБ для zip-архива, 10 ГБ для контейнерного образа).
- Сравнение Serverless и классических серверных (Server-based) решений:
  - В serverless разработчик освобождается от управления инстансами, масштабированием и обновлениями ОС, концентрируясь только на логике приложения.
  - В serverless разработчик освобождается от управления инстансами, масштабированием и обновлениями ОС, концентрируясь только на логике приложения.
- Экономика использования:
  - Для небольших или непредсказуемых рабочих нагрузок Lambda может быть существенно дешевле, чем поддержка виртуальных машин (EC2).
  - При большом постоянном трафике - классические серверные решения могут стать предпочтительнее.
