...
- _Push-модель (прямой вызов)_. Источник события непосредственно вызывает функцию, отправляя данные события. Примеры: S3, SNS, API Gateway.
- _Pull-модель (event source mapping)_. Lambda периодически опрашивает источник и извлекает события для обработки. Примеры: DynamoDB Streams, Kinesis, SQS.
