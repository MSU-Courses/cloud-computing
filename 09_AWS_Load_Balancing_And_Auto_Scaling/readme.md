# Балансировка нагрузки и автоматическое масштабирование. AWS ELB, EC2 AutoScaling

<i>
День выдался душным и тяжёлым. Воздух в аудитории стоял, как густой сироп, а ноутбуки гудели, будто усталые пчёлы. На экране Эммы мелькали строки логов, одна за другой, как удары сердца.

– Джон… – прошептала она. – Что-то происходит.

Он обернулся, и в свете монитора его лицо казалось почти призрачным.
– Покажи.

Вкладка с мониторингом дрожала от всплесков графиков. Красные линии поднимались всё выше, будто температура перед бурей.

– Это не может быть... – пробормотал он. – Мы же только вчера развернули всё по правилам.

Эмма нервно рассмеялась. – Наверное, нас заметили. Смотри, трафик идёт со всех континентов. Азия, Европа... даже кто-то из Исландии!

Сервер не выдержал. Экран на секунду погас, вентилятор взвыл, и лампа в углу моргнула.
Джон вскочил, будто это могло помочь машине выжить.

– Всё рушится, – сказала Эмма, глядя, как ошибки множатся, словно снежная лавина.

Он подошёл ближе, положил ладонь на стол, словно пытаясь остановить этот поток.
– Нет, – тихо сказал он. – Мы просто должны дать им подмогу.

Пальцы быстро забегали по клавиатуре.
– Auto Scaling… балансировка нагрузки… пусть каждый сервер примет часть удара.

На экране появились новые строки: Launching instance... Health check passed... Load balancer active.

Секунду спустя графики начали снижаться. Красные пики сменялись ровными зелёными полосами.

Эмма выдохнула, улыбнувшись:
– Кажется, шторм прошёл.

Джон откинулся на спинку стула. За окном уже вечерело, и где-то вдалеке, над университетом, плавно проплывали облака — спокойные, равномерные, словно отражение их системы.

– Просто нужно было разделить нагрузку, – сказал он. – Даже облакам иногда нужно несколько небес.
</i>

## Содержание

- [Балансировка нагрузки и автоматическое масштабирование. AWS ELB, EC2 AutoScaling](#балансировка-нагрузки-и-автоматическое-масштабирование-aws-elb-ec2-autoscaling)
  - [Содержание](#содержание)
  - [Вопросы для самопроверки](#вопросы-для-самопроверки)
  - [Введение. Проблема масштабирования в современных приложениях](#введение-проблема-масштабирования-в-современных-приложениях)
  - [Основы балансировки нагрузки](#основы-балансировки-нагрузки)
    - [Что такое балансировка нагрузки](#что-такое-балансировка-нагрузки)
    - [Зачем нужна балансировка нагрузки](#зачем-нужна-балансировка-нагрузки)
  - [Amazon Elastic Load Balancing (ELB)](#amazon-elastic-load-balancing-elb)
    - [Обзор сервиса Amazon ELB](#обзор-сервиса-amazon-elb)
    - [Принцип работы ELB](#принцип-работы-elb)
    - [Типы балансировщиков нагрузки в AWS](#типы-балансировщиков-нагрузки-в-aws)
    ...
    - [Компоненты Elastic Load Balancing](#компоненты-elastic-load-balancing)
      - [Listeners (Прослушиватели)](#listeners-прослушиватели)
      - [Target Groups (Целевые группы)](#target-groups-целевые-группы)
      - [Health Checks (Проверки работоспособности)](#health-checks-проверки-работоспособности)
    - [Сценарии использования ELB](#сценарии-использования-elb)
  - [Мониторинг балансировщиков нагрузки (Amazon CloudWatch)](#мониторинг-балансировщиков-нагрузки-amazon-cloudwatch)
    - [Основы мониторинга. Сервис Amazon CloudWatch](#основы-мониторинга-сервис-amazon-cloudwatch)
    - [Для чего нужен мониторинг](#для-чего-нужен-мониторинг)
    - [Возможности CloudWatch](#возможности-cloudwatch)
    - [CloudWatch Alarms (Сигналы тревоги)](#cloudwatch-alarms-сигналы-тревоги)
  - [Масштабирование в облачных вычислениях](#масштабирование-в-облачных-вычислениях)
    - [Что такое масштабирование](#что-такое-масштабирование)
    - [Зачем нужно масштабирование](#зачем-нужно-масштабирование)
    - [Типы масштабирования: вертикальное и горизонтальное](#типы-масштабирования-вертикальное-и-горизонтальное)
    - [Какой тип масштабирования используется в облаке](#какой-тип-масштабирования-используется-в-облаке)
  - [Amazon EC2 Auto Scaling](#amazon-ec2-auto-scaling)
    - [Что такое Amazon EC2 Auto Scaling](#что-такое-amazon-ec2-auto-scaling)
    - [Auto Scaling Group (Группа автомасштабирования)](#auto-scaling-group-группа-автомасштабирования)
    - [Scaling Out и Scaling In](#scaling-out-и-scaling-in)
    - [Как работает Amazon EC2 Auto Scaling](#как-работает-amazon-ec2-auto-scaling)
      - [Что запускать?](#что-запускать)
      - [Где запускать?](#где-запускать)
      - [Когда масштабировать?](#когда-масштабировать)
    - [Стратегии масштабирования в EC2 Auto Scaling](#стратегии-масштабирования-в-ec2-auto-scaling)
      - [Поддержание текущего количества инстансов (Maintain current instance count)](#поддержание-текущего-количества-инстансов-maintain-current-instance-count)
      - [Ручное масштабирование (Manual scaling)](#ручное-масштабирование-manual-scaling)
      - [Запланированное масштабирование (Scheduled scaling)](#запланированное-масштабирование-scheduled-scaling)
      - [Динамическое масштабирование (Dynamic scaling)](#динамическое-масштабирование-dynamic-scaling)
      - [Predictive Scaling (Прогнозирующее масштабирование)](#predictive-scaling-прогнозирующее-масштабирование)
    - [Обязательно ли использовать Load Balancer с Auto Scaling](#обязательно-ли-использовать-load-balancer-с-auto-scaling)
    - [AWS Auto Scaling и EC2 Auto Scaling: в чем разница](#aws-auto-scaling-и-ec2-auto-scaling-в-чем-разница)
    - [Сценарии использования Auto Scaling](#сценарии-использования-auto-scaling)
  - [Стоимость использования ELB, CloudWatch и Auto Scaling](#стоимость-использования-elb-cloudwatch-и-auto-scaling)
    - [Стоимость Elastic Load Balancing](#стоимость-elastic-load-balancing)
    - [Стоимость CloudWatch](#стоимость-cloudwatch)
    - [Стоимость Auto Scaling](#стоимость-auto-scaling)
  - [Резюме](#резюме)
...
_Рисунок 4. Network Load Balancer распределяет TCP/UDP трафик с высокой производительностью и низкой задержкой_
...
_Рисунок 5. Архитектура применения Gateway Load Balancer в AWS_
...
## Мониторинг балансировщиков нагрузки (Amazon CloudWatch)

### Основы мониторинга. Сервис Amazon CloudWatch
...
![Рисунок 8. График нагрузки на веб-сайт Amazon в ноябре с пиковыми нагрузками конце месяца](../_images/09/08_amazon_november.png)

_Рисунок 8. График нагрузки на веб-сайт Amazon в ноябре с пиковыми нагрузками конце месяца_
...
![Рисунок 11. Принцип работы Auto Scaling: что масштабируется, где и когда](../_images/09/12_how_auto_scaling_works.png)

_Рисунок 11. Принцип работы Auto Scaling: что масштабируется, где и когда_
...
![Рисунок 12. Динамическое масштабирование на основе метрик CloudWatch](../_images/09/11_dynamic_scaling.png)

_Рисунок 12. Динамическое масштабирование на основе метрик CloudWatch_
