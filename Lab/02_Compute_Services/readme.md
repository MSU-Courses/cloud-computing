...
Работа состоит из двух частей: базовый и продвинутый уровень. Перейти из продвинутого к базовому уровню можно в любой момент, но вернуться обратно будет сложнее.
...
> [!NOTE]
> Всё, что понадобится для работы, разобрано в лекции 4 "Вычислительные сервисы. Amazon EC2 и machine identity" и в Hands-on 4.1, 4.2, 4.3. Если какой-то шаг непонятен, сначала загляните туда.
...
Профессиональный подход строится на одной идее: _единственный источник кода - это репозиторий, а сервер забирает код оттуда сам_. В этой части вы реализуете её без специальных инструментов, только с помощью `git` и SSH. Позже в курсе этот процесс будут выполнять автоматические системы (CI/CD), но устроены они точно так же: где-то запускается команда, которая берёт код из репозитория и выкладывает его на сервер.
...
3. Загрузите в репозиторий код приложения.
...
1. Создайте папку для приложения и пользователя `ec2-user` её владельцем:
...
2. Скачайте код из созданного репозитория в данную директорию:
   ...
   Вы должны увидеть те же файлы, что и в репозитории на GitHub: `public`, `src`, `views`, `composer.json` и другие (или файлы вашего проекта).

3. Создайте файл настроек `.env`. В нём хранятся пароль к базе данных и другие параметры, которые нельзя класть в репозиторий. Откройте файл в редакторе `nano`:

   ```bash
   nano /var/www/timecapsule/.env
   ```

   Вставьте в него содержимое, подставив свой IP-адрес и пароль к базе из задания 9:
   ...
   Файл `.env` указан в `.gitignore`, поэтому git его не отслеживает: команда `git pull` его не тронет, а случайно отправить его в репозиторий не получится.
...
4. Настройте права доступа:
   ...
   Код приложения принадлежит пользователю `ec2-user`, под которым вы работаете. А PHP-FPM в Amazon Linux работает от имени пользователя `apache`. Первые две команды разрешают ему читать `.env`, но не дают читать этот файл посторонним. Третья команда отдаёт ему папку `storage`, куда приложение записывает загруженные файлы и сессии.

5. Установите библиотеки приложения и создайте таблицы в базе:
   ...
   `composer install` читает файл `composer.lock` и скачивает в папку `vendor` ровно те версии библиотек, которые в нём записаны. Флаг `--no-dev` пропускает библиотеки, нужные только при разработке. Скрипт `init-db.php` создаёт в базе недостающие таблицы и сообщает `Database is ready`.
...
3. Разберём этот файл построчно. Конфигурация nginx состоит из **директив**: каждая строка - это "название значение;", а фигурные скобки группируют директивы в блоки.
   ...
   | `root /var/www/timecapsule/public;`        | Папка, из которой берутся файлы. Из интернета доступна только папка `public`, поэтому код в `src`, библиотеки в `vendor` и файл `.env` никто снаружи открыть не сможет.                                                                                         |
...
4. Проверьте конфигурацию и запустите сервисы:
   ...
   sudo systemctl enable --now php-fpm # Включение и запуск PHP-FPM
   sudo systemctl restart nginx # Перезапуск nginx с новой конфигурацией
   ```
...
5. Откройте в браузере `http://<Public-IP>`: вы увидите страницу входа _TimeCapsule_. Затем откройте `http://<Public-IP>/health`. Этот адрес должен вернуть `{"status":"ok","db":"ok",...}`: приложение работает и видит базу данных.

### Задание 12. Обновление приложения

Приложение запущено. Теперь нужно научиться выпускать новые версии. Весь процесс состоит из четырёх шагов:

1. `git pull` - забрать новый код из репозитория.
2. `composer install` - обновить библиотеки, если их список изменился.
3. `php bin/init-db.php` - создать новые таблицы, если они появились.

Как именно выполнять эти шаги, зависит от специальности:

| Специальность        | Как выпускать новую версию                                |
| -------------------- | --------------------------------------------------------- |
| Informatică aplicată | Вариант A: выполнять команды на сервере вручную           |
| Informatică          | Вариант B: скрипт `deploy.sh`, запускаемый одной командой |
...
#### Вариант B. Скрипт `deploy.sh`

Выполнять одни и те же команды руками каждый раз неудобно, и легко что-нибудь пропустить. Поэтому их записывают в скрипт.

1. Создайте на сервере файл скрипта:
   ...
   ```bash
   #!/bin/bash
   # Выпуск новой версии TimeCapsule.
   set -euo pipefail
...
2. Разберём строки, которые встречаются впервые:

   | Строка                    | Что делает                                                                                                                                                               |
   | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
   | `#!/bin/bash`             | Указывает, какой программой выполнять файл: командной оболочкой `bash`.                                                                                                  |
   | `set -euo pipefail`       | Остановить скрипт при первой же ошибке. Без этой строки скрипт продолжил бы выполнять следующие команды, даже если предыдущая не сработала.                              |
   | `git pull --ff-only`      | Забрать новые коммиты, только если на сервере никто не менял файлы вручную. Если менял, `git pull` откажется работать, а не станет смешивать чужие правки с новым кодом. |
   | `echo "==> ..."`          | Напечатать, какой шаг сейчас выполняется. Если скрипт упадёт, будет видно, на каком шаге.                                                                                |
   | `$(git log -1 --oneline)` | Подставить в текст последний коммит: так видно, какая версия теперь работает на сервере.                                                                                 |
...
3. Сделайте скрипт исполняемым и запустите его:
   ...
   Новых коммитов пока нет, поэтому `git pull` сообщит `Already up to date`, а в последней строке вы увидите коммит, который сейчас работает на сервере.

4. Теперь запустите деплой прямо со своего компьютера, не заходя на сервер:

   ```bash
   ssh -i ~/.ssh/yournickname-keypair.pem ec2-user@<Public-IP> ./deploy.sh
   ```

   Если после адреса сервера в команде `ssh` написать команду, SSH выполнит её на сервере, покажет вывод и сразу отключится. Именно так устроены самые простые системы автоматического деплоя.
...
Теперь проверим весь процесс на практике.

1. _Проверьте приложение_. Зарегистрируйтесь и создайте капсулу с прикреплённым файлом. Для проверки можно выбрать дату открытия через пару минут.

2. _Выпустите новую версию_. На своём компьютере измените что-нибудь заметное, например добавьте в подвал страницы (файл `views/layout.php`) строку со своим именем. Отправьте изменение в репозиторий:
   ...
   Выполните деплой: вручную (вариант A) или скриптом (вариант B). Обновите страницу и убедитесь, что изменение появилось, а ваша капсула с файлом никуда не делась.

3. _Сломайте сайт_. Добавьте в конец файла `views/layout.php` строку с синтаксической ошибкой, например `<?php broken(`, отправьте её в репозиторий и снова выполните деплой. Откройте главную страницу: вы увидите ошибку 500. Теперь откройте `/health`.
...
4. _Откатитесь_. Исправлять код прямо на сервере нельзя: при следующем деплое `git pull` вернёт ошибку обратно, а изменения на сервере помешают ему выполниться. Откат делается через репозиторий. На своём компьютере выполните:
   ...
   Команда `git revert` создаёт новый коммит, который отменяет изменения последнего коммита. История при этом не переписывается: в ней видно, что ошибку внесли и затем отменили.

   Выполните деплой ещё раз и убедитесь, что сайт снова работает.
...
2. _Запишите архитектурное решение_. **Architecture Decision Record (ADR)** - короткий документ, в котором команда фиксирует одно важное решение: в какой ситуации его приняли, что выбрали, какие были альтернативы и к каким последствиям это приведёт. Через полгода по нему можно понять, почему система устроена именно так, даже если автора решения уже нет в команде.
   ...
   В своём ADR обязательно сравните как минимум три способа доставки кода: копирование через `scp`, `git pull` вручную на сервере и скрипт `deploy.sh`.

### Если вы выбрали Node.js, Laravel или своё приложение

Порядок работы тот же, меняются только команды установки и запуска. Студенты Informatică пишут `deploy.sh` с теми же шагами, но с командами своего приложения.
...
- Установка: `sudo dnf -y install nodejs22 git postgresql16-server`, затем `sudo npm install -g pm2`. **PM2** - менеджер процессов для Node.js: он держит приложение запущенным и перезапускает его после сбоя или перезагрузки сервера. Пакеты PHP из задания 8 не нужны.
- После установки проверьте версии командами `node -v` и `npm -v`.
- Задание 9 выполняется так же. В задании 10 не меняйте владельца `storage` и `.env` на `apache`: приложение под PM2 работает от имени `ec2-user`.
- Первый запуск: `npm ci --omit=dev`, `npm run db:init`, `pm2 start ecosystem.config.cjs`, `pm2 save` и `pm2 startup` (последняя команда выведет ещё одну команду, выполните и её).
- Обновление: `git pull`, `npm ci --omit=dev`, `npm run db:init`, `pm2 restart timecapsule`.
...
_Laravel_ (`open_capsules_php_laravel`). Дополнительно установите `php8.4-intl php8.4-zip`. После `composer install` выполните `php artisan key:generate`, `php artisan migrate --force` и `php artisan config:cache`, а права на запись дайте папкам `storage` и `bootstrap/cache`. При обновлении вместо `php bin/init-db.php` выполняйте `php artisan migrate --force` и `php artisan config:cache`. Точный список шагов есть в README приложения.

_Своё PHP-приложение_. Уберите `composer install`, если в проекте нет Composer, и замените `php bin/init-db.php` своим способом создания таблиц. Если приложение использует MySQL, установите вместо PostgreSQL пакет `mariadb105-server`.
...
| Ошибка 404 на всех страницах                                               | Существует ли папка `/var/www/timecapsule/public`? Проверьте `ls -l /var/www/timecapsule`. Совпадает ли путь в строке `root` конфигурации nginx.                           |
...
| Не загружается файл, ошибка про права                                      | Владелец папки `storage` должен быть `apache`: `ls -l /var/www/timecapsule/storage`.                                                                                       |
...
| Страница показывает `vendor/ is missing`                                   | Не выполнен `composer install` в папке `/var/www/timecapsule`.                                                                                                             |
| `git pull` пишет `Not possible to fast-forward` или `would be overwritten` | На сервере вручную меняли файлы из репозитория. Посмотрите их командой `git status` и отмените изменения: `git checkout -- <файл>`. Код меняется только через репозиторий. |
...
4. Для продвинутого уровня: ссылка на репозиторий, схема архитектуры и ADR из задания 14.
5. Вывод.
...
3. Скриншоты продвинутого уровня:
   ...
   2. Задание 10: вывод `php bin/init-db.php`.
   3. Задание 11: вывод `sudo nginx -t`, приложение в браузере и ответ `/health`.
   4. Задание 12, вариант B: вывод `deploy.sh`, запущенного со своего компьютера командой `ssh`.
   5. Задание 13: новая версия с вашим изменением на странице.
   6. Задание 13: ошибка 500 после сломанного деплоя и работающий сайт после `git revert` и повторного деплоя.
4. Архитектура (продвинутый уровень):
   1. Схема из задания 14, вставленная в отчёт картинкой. Образец оформления - рисунок 4.11 "Первая версия архитектуры учебного проекта" в лекции 4. Нарисуйте схему своего развёртывания, а не копию образца.
