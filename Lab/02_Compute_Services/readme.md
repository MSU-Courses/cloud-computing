# Лабораторная работа №2. Вычислительные сервисы. Деплой приложения на Amazon EC2
...
Кухня берёт продукты со склада (PostgreSQL), готовит блюдо и отдаёт его администратору, а тот выносит его посетителю.
...
Две последние команды меняют владельцев папок:

1. Код приложения принадлежит пользователю `ec2-user`, под которым вы работаете. 
2. PHP-FPM в Amazon Linux работает от имени пользователя `apache`, и ему нужно право записывать загруженные файлы и файлы сессий. Поэтому папка `storage` принадлежит `apache`.
...
1. _Нарисуйте схему_ своего развёртывания по правилам AWS, используя официальные иконки [AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) (например, в [draw.io](https://app.diagrams.net), где они уже есть в библиотеке). На схеме должны быть:
...
Для продвинутого уровня отчёт в формате Markdown кладётся в репозиторий приложения в организации GitHub, там же лежат схема архитектуры и ADR. Пригласите преподавателя в репозиторий с правами на чтение (`Settings` → `Collaborators and teams`).
...
Не публикуйте в отчёте приватные ключи, пароли и содержимое файла `.env`. Если они попали на скриншот, закройте их.
